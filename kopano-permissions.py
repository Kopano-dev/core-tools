...
def opt_args():
    ...
    parser.add_option("--permission", dest="permission", action="store",
                      help="Set permssions [norights|readonly|secretary|owner|fullcontrol] or use the calculate "
...
def addpermissions(user, options):
    ...
    if options.folders:
        foldername = user.store.folder(options.folders[0])
    else:
        foldername  = user.store
